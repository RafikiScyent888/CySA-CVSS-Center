...
/* =====================================================================
   ...
   question's own tile and the control to change, and offers a clean field —
   the commonest reason a stuck student stays stuck is their own wrong pick
   sitting in the box they are staring at.

   The mark never names the answer. It says where to look and where to act,
   and the student still has to make the call.
   ===================================================================== */
let litNodes = [];
...
/* The evidence lives in the question's OWN tile, so it is found rather than
   listed. A tile added later is covered on the day it is written instead of
   falling silently out of the feature. */
const EVIDENCE = ".logwrap, .metrics, .rulebox, .chain, .diamond, .gloss";
function highlight(card, where) {
  ...
  marks.forEach((n, i) => {
    n.classList.add("lit");
    /* A printed word, not just a glow: colour alone says nothing to a
       student who cannot separate it from the panel behind it, and several
       things glowing with no label says nothing to anybody. */
    ...
    wrong: (list) => {
      ...
      if (opts.onReset) {
        ...
        again.addEventListener("click", () => {
          opts.onReset();
          /* The hints stay, and so does the wrong-count. Nothing earned is
...
function renderQuestions(host, qs, onGraded) {
  ...
  qs.forEach((q) => {
    ...
    const hints = hintBox(card, {
      ...
      onReset: () => {
        /* Clears the attempt, not the progress. The feedback goes with the
           pick that produced it — "Not yet" over an empty box reads as a
           verdict on nothing. */
        input.value = "";
    ...
    function check() {
      ...
      if (ok) hints.right();
      else hints.wrong(q.hints ? q.hints() : questionHints(q.id, G));
      updateScore();
    }
