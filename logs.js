  ...
  if (G.fw.shape === "client") {
    ...
    for (var a = 0; a < G.fw.attempts; a++) {
      ...
      rows.push({
        ...
        /* Collapsible for display. The 200s are never folded — the rows that
           prove the breach have to stay on screen no matter how loud the
           attack was, or the aggregation would hide the finding. */
        _stream: !landed
      });
    }
...
export function siemRows(G) {
  ...
  if (G.siem.clientBorne) {
    ...
    G.siem.failTimes.forEach(function (ft) {
      rows.push({
        t: ft, eid: 4625, account: v.user, host: v.host,
        src: G.adversary.c2ip, geo: G.siem.geoAnomaly,
        result: "FAILED (bad password)", _t: "fail", _stream: true
      });
    });
  } else {
    /* Times come from the generator, not from here, because the correlation
       rule the student tunes runs over exactly these timestamps. Rolling
       them again locally would give the tile one stream and the grader
       another. `_stream` marks the rows the SIEM tile is allowed to
       collapse for display — nobody should scroll ninety-three of them. */
    G.siem.failTimes.forEach(function (ft) {
      rows.push({
        t: ft, eid: 4625, account: r.pick([v.user, "administrator", "svc_backup", "guest"]),
